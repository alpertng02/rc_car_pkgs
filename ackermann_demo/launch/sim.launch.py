...
def generate_launch_description():
    ...
    ackermann_astar_planner_node = Node(
        ...
        output='screen', parameters=planner_params, condition=nav_and_ackermann
    )

    # Legacy planner, selected with planner:=legacy.
    astar_planner_node = Node(
        package='ackermann_demo', executable='astar_planner', name='astar_planner',
        output='screen', parameters=planner_params, condition=nav_and_legacy
    )
    ...
    voronoi_costmap_node = Node(
        ...
        parameters=[{
            ...
            'd_o_max': 0.7,
            'alpha': 0.3,
            'min_obstacle_seperation': 0.4,
            'treat_unknown_as_obstacle': False,
            'border_is_obstacle': False
        }]
    )
    ...
    stanley_controller_node = Node(
        ...
        parameters=[{
            ...
            'stall_timeout_sec': 2.0,             # Allowed time window of zero acceleration/movement before path dropout
            'collision_accel_threshold': 4.0      # IMU forward-acceleration spike that is treated as an impact [m/s²]
        }]
    )
